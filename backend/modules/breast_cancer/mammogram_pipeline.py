...
try:
    if os.path.exists(MODEL_PATH):
        ...
        print(f"Mammogram CNN loaded from {MODEL_PATH}")
    else:
        model = None
        print(f"Warning: Mammogram Model not found at {MODEL_PATH}. Using mock predictions.")
except Exception as e:
    print(f"Error loading Mammogram CNN model: {e}")
    model = None

def process_mammogram(file_path: str) -> dict:
    start_time = time.time()
    filename = os.path.basename(file_path)
    image = cv2.imread(file_path)
    
    if image is None:
        raise ValueError("Invalid mammogram image file.")

    heatmap_path = f"results/mammo_heatmap_{filename}"
    ...
    if model is not None:
        ...
        confidenceScore = round(random.uniform(0.65, 0.98), 4)
    ...
    return {
        ...
        "heatmap_url": f"/static/mammo_heatmap_{filename}",
